...
#define Iqq _p[2]
#define Iqq_columnindex 2
#define gq _p[3]
#define gq_columnindex 3
#define qq _p[4]
#define qq_columnindex 4
#define Dqq _p[5]
#define Dqq_columnindex 5
#define v _p[6]
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt) {int _reset = 0; {
   Dqq = ( 1.0 - qq ) * alpha ( _threadargscomma_ v + shift ) - qq * beta ( _threadargscomma_ v + shift ) ;
   }
 return _reset;
}
 static int _ode_matsol1 (double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt) {
 Dqq = Dqq  / (1. - dt*( ( ( ( - 1.0 ) ) )*( alpha ( _threadargscomma_ v + shift ) ) - ( 1.0 )*( beta ( _threadargscomma_ v + shift ) ) )) ;
  return 0;
}
 /*END CVODE*/
 static int state (double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt) { {
    qq = qq + (1. - exp(dt*(( ( ( - 1.0 ) ) )*( alpha ( _threadargscomma_ v + shift ) ) - ( 1.0 )*( beta ( _threadargscomma_ v + shift ) ))))*(- ( ( ( 1.0 ) )*( alpha ( _threadargscomma_ v + shift ) ) ) / ( ( ( ( - 1.0 ) ) )*( alpha ( _threadargscomma_ v + shift ) ) - ( 1.0 )*( beta ( _threadargscomma_ v + shift ) ) ) - qq) ;
   }
  return 0;
}
...
static void initmodel(double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt) {
  ...
  qq = qq0;
 {
   qq = alpha ( _threadargscomma_ v + shift ) / ( beta ( _threadargscomma_ v + shift ) + alpha ( _threadargscomma_ v + shift ) ) ;
   }
...
static void _initlists(){
 ...
 int _i; static int _first = 1;
  if (!_first) return;
 _slist1[0] = qq_columnindex;  _dlist1[0] = Dqq_columnindex;
_first = 0;
}
